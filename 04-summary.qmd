...
The data are in a `r glossary("CSV")` file, so we can read this with the function `read_csv()` , and assign it to a new object that we'll call `births`. You can learn more about importing data from other file types in @sec-data. 
...
It looks like this column doesn't contain any useful info, so we can just ignore it, or skip it using the `col_types` argument and two helper functions, `cols()` and `col_skip()`. 

You can also set the order of levels for `r glossary("categorical")` data when you first import it. You can also use the `count()` function to check the level labels for `AgeGroup`, `Delivery` and `Induced`; set `sort = TRUE` to sort by frequency.
...
To create the column specification object, just edit the output of `spec(births)`; you can remove the columns that imported correctly, or leave them all in for clarity. Setting the `col_types` argument also removes the message about column types.

```{r}
# edit the output of spec(births)
birth_cols <- cols(
  SIMDQuintileQF = col_skip(),
  AgeGroup = col_factor(levels = c("25-34", "35 and over", "Under 25", "Unknown")),
...
Data sets often import with column names that are a little difficult to use in code. You can manually rename them, but the `pkg("janitor")` package makes this incredibly easy. This dataset has pretty good names, with no spaces or special characters, but it does use uppercase letters to separate words, which can be hard to remember. the `clean_names()` function changes everything to lowercase and uses underscores to separate word parts. 
...
You can also use additional functions like `filter()` or `mutate()` after `group_by` or with the `.by` argument. You'll learn more about these in @sec-wrangle but briefly:
...
* transform the count into a percentage of the births per year and age group


```{r}
births_per_year_type_age <- births %>%
  mutate(year = str_extract(financial_year, ".{4}"),
...
* What other insights can you generate about the birth data?
* Download some other data from [Scottish Health and Social Care Oopen Data](https://www.opendata.nhs.scot/) and create summary tables and plots.
* Summarise your own data.
