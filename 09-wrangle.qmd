...
## Intended Learning Outcomes {#sec-ilo-wrangle}
...
## Walkthrough video {#sec-walkthrough-wrangle}
...
Data wrangling refers to the process of cleaning, transforming, and restructuring your data to get it into the format you need for analysis and it's something you will spend an awful lot of time doing. Most `r glossary("data wrangling")` involves the reshaping functions you learned in @sec-tidy and six functions from the `r pkg("dplyr")` package that is loaded as part of the `r pkg("tidyverse")`: `select`, `filter`, `arrange`, `mutate`, `summarise`, and `group_by`. You'll remember the last two from @sec-summary, so we'll only cover them briefly.
